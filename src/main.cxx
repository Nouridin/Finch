...
main (int argc, char* argv[])
{
        ...
        {
                std::cerr << "[ERROR] Failed to bind socket to " << config.listen_ip << ":"
                          << config.listen_port << "\n";
        ...
        std::cout << " Latency      : " << config.delay_ms << "ms (+/-" << config.jitter_ms
                  << "ms jitter)\n";
        ...
        std::priority_queue<DelayedPacket, std::vector<DelayedPacket>, std::greater<DelayedPacket>>
            packet_queue;
        ...
        while (true)
        {
                ...
                int bytes_received = recvfrom(proxy_sock, (char*)buffer, sizeof(buffer), 0,
                ...
                if (bytes_received > 0)
                {
                        ...
                        bool is_from_target =
                            (sender_addr.sin_addr.s_addr == target_addr.sin_addr.s_addr &&
                        ...
                        if (is_from_target)
                        {
                                if (!client_connected)
                                {
                                        std::cout << "[WARN] Packet received from target, but "
                                                     "client endpoint unknown.\n";
                                        continue;
                                }
                                destination_addr = last_known_client_addr;
                                std::cout << "[REPLY] Target -> Client (" << bytes_received
                                          << " bytes)\n";
                        }
                        else
                        {
                                ...
                                std::cout << "[FWD] Client -> Target (" << bytes_received
                                          << " bytes)\n";
                        }
                        ...
                        {
                                dropped_packets++;
                                std::cout << "[DROP] Packet #" << total_packets << " ("
                        ...
                        else
                        {
                                ...
                                corruptPayload(buffer, (size_t)bytes_received,
                                               (float)config.bitflip_rate, rng);
                                ...
                                if (config.jitter_ms > 0)
                                {
                                        std::uniform_int_distribution<int32_t> jitter_dist(
                                            -(int32_t)config.jitter_ms, (int32_t)config.jitter_ms);
                                ...
                                auto release_time = std::chrono::steady_clock::now() +
                                                    std::chrono::milliseconds(applied_delay);
                                packet_queue.push(
                                    {std::vector<uint8_t>(buffer, buffer + bytes_received),
                                     release_time, total_packets, destination_addr});
                        }
                }
                ...
                while (!packet_queue.empty() && packet_queue.top().release_time <= now)
                {
                        ...
                        sendto(proxy_sock, (const char*)pkt.payload.data(), (int)pkt.payload.size(),
                               0, (struct sockaddr*)&pkt.dest_addr, sizeof(pkt.dest_addr));
                }
